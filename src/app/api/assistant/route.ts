...
• **דוחות** (/reports) — 3 טאבים: **דוח מועסקים** (/reports?tab=hired) כל מי שהתקבל/התחיל, סינון מעסיק/תאריכים; **דוח מקדמות** (/reports?tab=advances) רישום ומעקב מקדמות שכר לעובדים מועסקים (כפתור "רישום מקדמה": עובד, סכום, תאריך), סיכום לפי עובד; **דוח העברות בין עבודות** (/reports?tab=transfers) מי עבר ממעסיק/תפקיד למעסיק אחר ומתי — נרשם אוטומטית כשמשנים מעסיק לעובד מועסק, או ידנית ("רישום העברה"). בכל טאב יש "ייצוא לאקסל". אני יכול/ה גם ליצור קישור CSV (export_leads_csv / get_hired_report).
